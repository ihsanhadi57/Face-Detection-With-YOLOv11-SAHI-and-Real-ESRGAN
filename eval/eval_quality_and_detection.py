...
# --- IQA HELPER FUNCTIONS ---

def load_iqa_metric(metric_name, device):
    """Loads a single IQA metric."""
    try:
        return pyiqa.create_metric(metric_name, device=device)
    except Exception as e:
        print(f"Warning: Could not load IQA metric '{metric_name}': {e}")
        return None

def get_available_metrics(device):
    """Detect and load available IQA metrics."""
    available = {}
    metrics_to_try = {
        'niqe': {'name': 'NIQE', 'inverse': True},
        'brisque': {'name': 'BRISQUE', 'inverse': True},
        'topiq_nr-face': {'name': 'TOPIQ-Face', 'inverse': False},
    }
    ...
    for key, info in metrics_to_try.items():
        ...
        if metric is not None:
            available[key] = {'metric': metric, 'name': info['name'], 'inverse': info['inverse']}
            print(f"   ✓ Metric loaded: {info['name']}")
            
    if not available:
        print("   ❌ No IQA metrics could be loaded. Please ensure 'pyiqa' is installed correctly.")
        
    return available

def calculate_iqa_scores(image_np, device, available_metrics):
    """Calculates IQA scores for a given image."""
    scores = {}
    if not available_metrics or image_np is None or image_np.size == 0:
        return scores

    try:
        # Preprocessing for pyiqa
        if image_np.max() > 1.0:
        ...
        with torch.no_grad():
            for metric_key, metric_info in available_metrics.items():
                try:
                    ...
                    # Could fail on very small or unusual images, skip metric
                    continue
    except Exception:
        # General preprocessing error, return empty scores
        pass
...
class SAHIWiderFaceEvaluator:
    def __init__(self, 
                 ...
                 eval_quality=False): # NEW: Flag untuk evaluasi kualitas
        ...
        self.iqa_scores = {'original': defaultdict(lambda: defaultdict(list)), 
                           'enhanced': defaultdict(lambda: defaultdict(list))}
        if self.eval_quality:
            self.iqa_metrics = get_available_metrics(self.device)
        ...
        if self.use_enhancer:
            ...
            try:
                # Selalu gunakan x2 untuk konsistensi, atau bisa dibuat argumen
                ...
                print(f"   ❌ Failed to load Enhancer model: {e}. Disabling enhancement.")
    ...
    def run_inference_on_images(self, category):
        ...
        if self.use_enhancer:
            mode_str = f"{'Bounded' if self.bounded_enhancement else 'Full'}Enhance -> {mode_str}"
        ...
        predictions = {}
        category_stats = {'enhanced': 0, 'skipped': 0, 'total': len(image_files)}
        ...
        for img_file in tqdm(image_files, desc=f"{mode_str} {category}"):
            ...
            original_image_bgr = cv2.imread(original_img_path)
            if original_image_bgr is None: 
                continue

            # --- IQA on Original Image ---
            if self.eval_quality and self.iqa_metrics:
                original_image_rgb = cv2.cvtColor(original_image_bgr, cv2.COLOR_BGR2RGB)
                scores = calculate_iqa_scores(original_image_rgb, self.device, self.iqa_metrics)
                for key, score in scores.items():
                    self.iqa_scores['original'][category][key].append(score)

            self.enhancement_stats['total_images'] += 1
            
            # --- ENHANCEMENT LOGIC ---
            should_enhance_flag = False
            if self.use_enhancer and self.face_enhancer:
                if self.bounded_enhancement:
                    should_enhance_flag, reason, _ = self.quick_face_analysis(original_img_path)
                    self.enhancement_stats['enhancement_decisions'].append({
                        'image': img_file.name, 'category': category,
                        'should_enhance': should_enhance_flag, 'reason': reason})
                else: # Full enhancement
                    should_enhance_flag = True

                if should_enhance_flag:
                    enhanced_image_bgr, success = self.face_enhancer.enhance_image(original_image_bgr)
                    if success:
                        temp_img_name = f"enhanced_{img_file.name}"
                        enhanced_img_path = str(self.temp_enh_dir / temp_img_name)
                        cv2.imwrite(enhanced_img_path, enhanced_image_bgr)
                        inference_path = enhanced_img_path
                        was_enhanced = True
                        category_stats['enhanced'] += 1
                        self.enhancement_stats['enhanced_images'] += 1
                        
                        # --- IQA on Enhanced Image ---
                        if self.eval_quality and self.iqa_metrics:
                            enhanced_image_rgb = cv2.cvtColor(enhanced_image_bgr, cv2.COLOR_BGR2RGB)
                            scores = calculate_iqa_scores(enhanced_image_rgb, self.device, self.iqa_metrics)
                            for key, score in scores.items():
                                self.iqa_scores['enhanced'][category][key].append(score)
                else:
            ...
            if was_enhanced:
                ...
                for pred in pred_boxes:
                    pred['bbox'] = [round(b / scale) for b in pred['bbox']]
                os.remove(inference_path)
            
            predictions[img_file.name.replace('_', '/', 1)] = pred_boxes
        
        # Print category statistics
        total_faces = sum(len(p) for p in predictions.values())
        if self.bounded_enhancement and self.use_enhancer:
            ...
            print(f"   ✓ Enhancement: {category_stats['enhanced']}/{category_stats['total']} images ({enhance_ratio:.1f}%) | Faces: {total_faces}")
        else:
    ...
    def _print_iqa_summary(self):
        """Prints a summary of the collected IQA scores."""
        if not self.eval_quality or not self.iqa_metrics:
            return

        print("\n" + "="*80)
        print("⭐ IMAGE QUALITY ASSESSMENT (IQA) SUMMARY")
        print("="*80)

        metric_names = [info['name'] for info in self.iqa_metrics.values()]
        header = f"{'Category':<20} | {'Image Type':<12} | " + " | ".join([f"{name:<12}" for name in metric_names])
        print(header)
        print("-" * len(header))

        for category in self.categories + ['overall']:
            for img_type in ['original', 'enhanced']:
                if not self.use_enhancer and img_type == 'enhanced':
                    continue

                line = f"{category.replace('_', ' ').title():<20} | {img_type.title():<12} | "
                
                scores_to_print = []
                for metric_key, metric_info in self.iqa_metrics.items():
                    
                    if category == 'overall':
                        # Flatten scores from all categories
                        all_scores = [s for cat_scores in self.iqa_scores[img_type].values() for s in cat_scores.get(metric_key, [])]
                    else:
                        all_scores = self.iqa_scores[img_type][category].get(metric_key, [])

                    if all_scores:
                        avg_score = np.mean(all_scores)
                        scores_to_print.append(f"{avg_score:<12.3f}")
                    else:
                        scores_to_print.append(f"{'N/A':<12}")
                
                print(line + " | ".join(scores_to_print))

            if category != 'overall':
                print("-" * len(header))
        print("="*80)
        print("Lower is better for NIQE, BRISQUE. Higher is better for TOPIQ-Face.")
    ...
    def run(self):
        ...
        self._print_iqa_summary()  # NEW: Print IQA summary
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Evaluate YOLO on WIDER Face with Detection and Quality metrics.')
    parser.add_argument('--mode', choices=['standard', 'sahi-uniform', 'sahi-adaptive'], default='sahi-uniform', help='Inference mode')
    parser.add_argument('--enhance', action='store_true', help='Enable Enhancement pipeline')
    parser.add_argument('--bounded', action='store_true', help='Enable Bounded (selective) Enhancement')
    parser.add_argument('--quality', action='store_true', help='Enable Image Quality Assessment (IQA)') # NEW
    parser.add_argument('--threshold', type=int, default=50, help='Face size threshold for bounded enhancement')
